...
import { ZERO_HASH, type SimulCore } from "./core";

export const FOURK_MODE_BYTE = 1;

/** Share-code section body: varint round, a slot bitmask (bit0/1 commitment
 * 1/2 present, bit2/3 reveal 1/2 present), then each present commitment
 * (32 bytes) and each present reveal (1 byte, column + 1). Zero slots are
 * omitted — the common between-rounds case costs 3 bytes. */
export function pushSimulSection(out: number[], core: SimulCore | undefined): void {
  ...
  out.push((c1 ? 1 : 0) | (c2 ? 2 : 0) | (c.reveal1 ? 4 : 0) | (c.reveal2 ? 8 : 0));
...
export function takeSimulSection(buf: Uint8Array, i: { p: number }): SimulCore {
  ...
  if (slots & 0xf0) throw new Error("bad slot mask in share code");
  ...
  return { round, commit1, commit2, reveal1, reveal2 };
}
